...
-(instancetype)init
{
    ...
    if(self)
    {
        ...
        __weak ZSDPaymentView *weakSelf = self;
        ...
        _paymentForm.completeHandle = ^(NSString *inputPwd)
        {
            ...
            {
                NSLog(@"密码是6为数字");
                if (weakSelf.delegate && ([weakSelf.delegate respondsToSelector:@selector(passwordInputCompleted:)])) {
                    [weakSelf.delegate passwordInputCompleted:inputPwd];
                }
            }
            else
            {
...
-(void)keyboardWillShow:(NSNotification *)notificaiton
{
    ...
    if((_paymentForm.frame.origin.y+10) != _keyboardOriginY)
    {
...
-(void)resetAlertFrame
{
    ...
    if(bottom < keyboardHeight)
    {
        ...
        alertFrame.origin.y = alertFrame.origin.y -10;
